...
const ServiceCards = ({arr1 , arr2 , condition}) => {
  return (
    <>
    <div>
      <div className='mx-6 md:mx-[10rem] lg:mx-[15rem] m-auto text-center space-y-8'>
      {
       arr1.map((item , index)=>{
        return(
          <div key={index}>
            <div className='text-[2rem] md:text-[3rem] font-bold leading-[2.8rem] md:leading-[3.7rem]'>{item.h}</div>
            <div className='text-[#989DA6] text-[15px] py-4'>{item.desc}</div>
          </div>
        )
       })  
      }
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 px-3 xl:px-24'>
      {
       arr2.map((item , index)=>{
        return(
          <>
          <div className='border rounded-2xl m-4 px-9 py-12 space-y-6 hover:bg-[#FFB400]' key={index}>
      ...
      <div className='text-center mx-4 my-14 '>
